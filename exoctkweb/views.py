...
from exoctkweb import app_exoctk
...
def exoctk_ldc_results():
    ...
    if not hasattr(model_grid, 'data'):
        ...
        if len(model_grid.data) == 0:
            ...
            return render_template('ldc_error.html', teff=teff, logg=logg,
                                   feh=feh, \
                                   band=bandpass or 'None',
                                   profile=', '.join(profiles),
                                   models=model_grid.path, \
    ...
    if bandpass in ExoCTK.svo.filters()['Band'] or bandpass in ['tophat',
                                                                'NIRISS.GR700XD.1']:
        ...
        try:
            ...
            kwargs = {'n_bins': int(n_bins)} if n_bins else \
                {'pixels_per_bin': int(
                    pixels_per_bin)} if pixels_per_bin else {}
            ...
            if bandpass == 'NIRISS.GR700XD.1':
                p = os.path.join(os.path.dirname(ExoCTK.__file__),
                                 'data/filters/NIRISS.GR700XD.1.txt')
                bandpass = ExoCTK.svo.Filter(bandpass,
                                             filter_directory=np.genfromtxt(p,
                                                                            unpack=True),
                                             **kwargs)
            else:
    ...
    if not trim_rng:
        ...
        return render_template('ldc_error.html', teff=teff, logg=logg, feh=feh, \
                               band=bp_name, profile=', '.join(profiles),
                               models=model_grid.path, \
                               message=message)
    ...
    grid_point = ExoCTK.ldc.ldcfit.ldc(teff, logg, feh, model_grid, profiles,
                                       mu_min=mu_min, bandpass=bandpass,
                                       plot=False, colors=COLORS,
                                       save='output.txt')
    ...
    for profile in profiles:
        ...
        table.rename_column('wavelength',
    ...
    return render_template('ldc_results.html', teff=teff, logg=logg, feh=feh, \
                           ...
                           r=r_eff, models=model_grid.path,
...
def exoctk_tot_results():
    ...
    exo_dict['planet']['ars'] = float(
        request.form['ars'])  # Semi-major axis in units of stellar radii (a/R*)
    ...
    # Run PandExo
    deptherr, rms, ptsOrbit = ExoCTK.tot.transit_obs.run_pandexo(exo_dict,
                                                                 inst_dict,
                                                                 output_file='wasp43b_G141.p')
    ...
    binspec = ExoCTK.tot.transit_obs.plot_PlanSpec(specfile, w_unit, grism,
                                                   deptherr, nchan, smooth=10,
    ...
    ymin, ymax = (
    np.min(binspec) - 2 * deptherr, np.max(binspec) + 2 * deptherr)
    ...
    minphase, maxphase = ExoCTK.tot.transit_obs.calc_StartWindow('eclipse', rms,
                                                                 ptsOrbit,
                                                                 numorbits,
                                                                 depth, inc,
                                                                 aRs, period,
    ...
    return render_template('tot_results.html', exo_dict=exo_dict,
                           html_dict=html_dict)
...
def exoctk_tor_results():
    try:
        ...
        tor_output = create_tor_dict(float(obs_time), n_group, float(mag),
                                     str(band), str(filt), str(ins),
                                     str(subarray), str(sat_mode),
                                     float(sat_max), int(n_reset), infile)
        ...
        tor_err = 'This is not an error we anticipated, but the error caught was : ' + str(
            e)
        return render_template('tor_error.html', tor_err=tor_err)
...
def exoctk_tor2():
    ...
    if request.method == 'POST':
        ...
        contamVars['ra'], contamVars['dec'] = request.form['ra'], request.form[
            'dec']
        ...
        if request.form['submit'] == 'Calculate contamination':
            ...
            contamVars[
                'tyc-55'] = 'images/contamination-TYC_5530-1795-1_PA0-360.png'
...
def exoctk_filter_profile(ins):
    ...
    return render_template('tor_filter_profile.html', names=names,
                           filt_imgs=filt_imgs, ins=ins)
...
def exoctk_savefile():
    ...
    response.headers[
        "Content-Disposition"] = "attachment; filename=ExoXTK_results.txt"
    return response
...
def _param_validation(args):
    ...
    try:
        ...
        if P < 0:
            invalid[
                'P'] = "Cloud Pressure must be greater than or equal to zero"
            P = 0.0
    except ValueError:
    ...
    try:
        ...
        if Rayleigh <= 0:
            invalid[
                'Rayleigh'] = "Rayleight augmentation must be greater than zero"
            Rayleigh = 1.0
    except ValueError:
...
def exotransmit_portal():
    ...
    if exotransmit_dir is None:
        return render_template('tor_error.html',
                               tor_err="There seems to be no directory in place for exo-transmit...")
...
def fortney_portal():
    ...
    temp, chem, cloud, pmass, m_unit, reference_radius, r_unit, rstar, rstar_unit = _param_fort_validation(
        args)
    # get sqlite database
    ...
    if args:
        ...
        z_lambda = r_lambda - (1.25 * u.R_jup).to(
            u.km)  # all fortney models have fixed 1.25 radii

        # scale with planetary mass
        pmass = float(pmass)
        mass = (pmass * u.Unit(m_unit)).to(u.kg)
        gravity = constants.G * (mass) / (rplan.to(
            u.m)) ** 2.0  # convert radius to m for gravity units
